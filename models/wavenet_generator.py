...
class WavenetGenerator(nn.Module):
    ...
    def __init__(self, wavenet: WaveNet, batch_size=1, cond_repeat=800, wav_freq=16000):
        super().__init__()
        self.wavenet = copy.deepcopy(wavenet)
    ...
    def generate(self, encodings, init=True, method='sample'):
        ...
        samples = torch.zeros(encodings.size(0), 1, encodings.size(2) + 1)
        ...
        with torch.no_grad():
            t0 = time.time()
            for t1 in tqdm(range(encodings.size(2)), desc='Generating', ascii=True, miniters=1000, mininterval=20):
                t = t1
