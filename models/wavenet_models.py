...
class PitchRegressor(nn.Module):
    def __init__(self, args):
        ...
        convs = []
        for i in range(args.pitch_layers):
            in_channels = args.latent_d if i == 0 else args.pitch_channels
            convs.append(nn.Conv1d(in_channels, args.pitch_channels, kernel_size=3, stride=1, padding=1))
            convs.append(nn.ELU())
        convs.append(nn.Conv1d(args.pitch_channels, 1, 1))

        self.convs = nn.Sequential(*convs)
        self.dropout = nn.Dropout(p=args.p_dropout_discriminator)

    def forward(self, z):
        z = z.unsqueeze(3).repeat(1, 1, 1, 5)  # [B, C, L, 5]
        z = z.contiguous().view(z.size(0), z.size(1), -1)  #[B, C, 5L]

        z = self.dropout(z)
        out = self.convs(z)  # (B 1, L)
